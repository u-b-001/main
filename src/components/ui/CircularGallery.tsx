...
function deriveFontFamilyFromUrl(url: any) {
  const fileName = (url.split('/').pop() || 'custom-font').split('?')[0];
  const base = fileName.replace(/\.(woff2?|ttf|otf|eot)$/i, '');
  return base.replace(/[^a-zA-Z0-9-_ ]/g, '').trim() || 'CircularGalleryFont';
}

async function loadFontFromStylesheet(url: any) {
...
async function loadFontFromFile(url: any) {
...
async function loadCustomFont(fontUrl: any) {
...
async function resolveFont(font: any, fontUrl: any) {
...
class Media {
  ...
  update(scroll: any, direction: any) {
    ...
    let isHovered = false;
    if (this.app.mouse && this.app.mouse.x !== undefined) {
      const mouseWorldX = this.app.mouse.x * (this.viewport.width / 2);
      const mouseWorldY = this.app.mouse.y * (this.viewport.height / 2);
      const dx = mouseWorldX - this.plane.position.x;
      const dy = mouseWorldY - this.plane.position.y;
      const cos = Math.cos(-this.plane.rotation.z);
      const sin = Math.sin(-this.plane.rotation.z);
      const localX = dx * cos - dy * sin;
      const localY = dx * sin + dy * cos;
      if (
        Math.abs(localX) < this.plane.scale.x / 2 &&
        Math.abs(localY) < this.plane.scale.y / 2
      ) {
        isHovered = true;
      }
    }
    this.hoverValue = lerp(this.hoverValue || 0, isHovered ? 1 : 0, 0.1);
    this.plane.position.y += this.hoverValue * 0.1 * this.plane.scale.y;

    if (this.title && this.title.mesh && this.title.mesh.program) {
      this.title.mesh.program.uniforms.uHover.value = this.hoverValue;
    }
...
class App {
  ...
  createMedias(items: any, bend = 1, textColor: any, borderRadius: any, font: any) {
    ...
    this.medias = this.mediasImages.map((data: any, index: any) => {
      return new Media({
